...
池塘 Seed 预校准曾出现同一 254 步基础档有时成功、有时在非战斗画面执行名称 OCR。日志表明固定动作序列已经走完，问题集中在冲浪确认的第三次 `A`：原来的 800 ms 间隔可能让按键落在转场中。运行副本现将该窗口延长到 2000 ms，并在名称 OCR 前最多 4 次只等待重采样 `野生出现/抓捕就绪`；未确认进入战斗时本轮安全重启，同时打印每次标签分数，不会用补按键把错误状态继续推进，也不会把地图或菜单文字当作宝可梦名称。
