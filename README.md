...
- TID/SID 页接入英文/日文 1.3.7 脚本，支持乱数与穷举、中心帧/半径、固定延迟校准、取名、特殊号码、本地标签识别和御三家连续流程阶段包生成。
...
- 可选的 TID/SID 到御三家连续流程计划，包含游戏版本、御三家、ADV 搜索上下限和 SID ADV 重试半径；Seed 时间直接取 Ten Lines 对应设置的 Seed 表，不要求用户填写时间下限。
...
御三家阶段按普通乱数目标处理：尚未命中目标 PID 时继续原有自动校准，不判断也不修正 SID；命中目标 PID 后，不闪表示本次 SID 未命中，闪光表示 SID 命中并结束。SID 未命中后，以当前 `$SID_ADV修正` 为中心按 `当前、+1、-1、+2、-2...` 尝试。GUI 的 TID 页已经能生成 `01_id`、`02_lab_bridge` 和 `flow_plan.json`，并展示御三家 Seed、PID、IV、ADV 与重试顺序；第三阶段执行器接通前不会开放“开始运行”。完整三阶段自动启动及实机验收仍未完成。
...
截至 2026-08-13，现有测试共 79 项，使用项目 `.venv` 全部通过。它们覆盖计划择优、IV 输入、SID 采集请求与反查、御三家最低闪光目标、完整 Ten Lines Seed 时间表和 SID 判定、TID 连续流程双阶段预检、Seed 模式、静态目标白名单、路线边界、1.1.8 参数替换、孵蛋参数、TID/SID 1.3.7 日英模板与标签、标签兼容、EasyCon 1.6.4a 预检和 GUI/CLI 识图取整兼容 runner。测试通过不等于 Switch 实机长跑已经验收。
