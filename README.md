...
孵蛋总控把返回值 `2` 解释为“本轮已重启，继续下一轮”，把 `0` 解释为“不可恢复，停止整个流程”。原 1.1.8 时间轴脚本有几条瞬时失败分支虽然已经调用重启，却错误返回 `0`，所以会出现“游戏已经重启，脚本随后仍打印失败并结束”。生成副本现在把生成/领取/Seed 复核抓捕动作失败、领取后野生 Seed 反查失败和孵化动作失败改为返回 `2`；前两类按需要保留或重置 Seed 预校准状态后继续。HOME_BUFFER 无可用窗口、时间轴计算无效、亲本/目标数据导致蛋个体扩窗仍无候选等明确配置错误仍会停止，避免无意义死循环。
