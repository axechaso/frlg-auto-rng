...
池塘 Seed 预校准和领取后复核曾出现同一 254 步基础档有时成功、有时没有进入战斗。实机观察确认其中一种竞态是冲浪转场吞掉 `X`：随后原本用于选择队伍菜单的 `DOWN` 落到地图上，让人物向下移动一格。运行副本除把冲浪确认窗口从 800 ms 延长到 2000 ms 外，现在还会反复尝试 `X`，并且只有识别到 `三代菜单栏 > 90` 后才发送 `DOWN`；连续 5 次仍未打开菜单就安全重启。使用甜甜香气后还会在名称 OCR 前最多 4 次只等待重采样 `野生出现/抓捕就绪`，不会把地图或菜单文字当作宝可梦名称。
...
该页不会搜索方案、替换参数、复制所选脚本或套用 1.1.8 完整语料指纹，因此可以把同一个测试工程分别交给两种后端。日志包装器只负责原样转存终端输出；串口、采集卡、DSHOW 和 `build_run_command()` 与正式工具共用。页面还提供“准备内置冲浪菜单测试”，会从当前 1.1.8 包复制 `三代菜单栏`、`火红BAG` 两个标签到独立运行目录，用于测试冲浪后第一次 `X` 与稳定 BAG 标签，且不会发送方向键、抓捕或存档。
