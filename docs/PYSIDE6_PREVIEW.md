...
- 保留深色侧栏、白色卡片和固定底栏，默认 1280×860，最小 900×620。每页纵向滚动，宽表格内部横向滚动。共通设置通过顶部按钮打开，方案与预检结果独立展开。
- SID 补齐六槽、初始等级、野生地点、EV、糖果/阈值/机型/确认；TID 恢复正式七分区；实测表保留完整 11 列；野生/静态补齐身份、指定模式、觉醒力量和 SID 遍历；孵蛋补齐运行条件、亲本与四种配置操作；日志恢复设备标签诊断布局。
- 可以实际操作的内容仅为内存表单：导航、高级显隐、SID 槽位数量、IV 预设/单项重置、模式互斥、TID 条件禁用/语言默认值、野生与孵蛋的游戏/主机同步、结果区展开及共通设置窗口。
...
页面键为 `sid / tid / tid_records / wild / egg / script_test / logs`；`--scroll` 为 0–1 的纵向位置，`--settings` 改为打开并截图共通设置，`--advanced` 展示高级控件。`--page script_test` 会自动开启高级模式。截图命令实际 `show()` 窗口，在 Qt 布局稳定后保存 PNG 并退出；正常启动不带 `--screenshot`。
...
`tests/test_pyside_preview.py` 共 10 项定向检查，覆盖独立导入、PNG、页面上下文保留、六槽禁用、精确 IV 预设、野生互斥/共用身份、TID 接续/延迟、Seed 高级范围、后端按钮禁用、最小窗口与底部可达性。CI 安装可选 PySide6 依赖执行这些检查，正式程序依赖表不变。测试与视觉检查均不代表后端迁移或 Switch 实机验收完成。
