...
共通设置中的重复设备动作已移除：检测、虚拟手柄与监视窗口只保留在“运行准备”，窄屏仍通过“方案与设备”访问；路径选择、Seed 表更新与程序更新留在共通设置。所有后端动作仍未接入。
...
2026-09-06 增量核对至 `b33b57f`（含 `fb0fa99`）：多目标、接近距离 / 次数、自动切换后半径输入已存在，默认值与启用条件未变。本次同步局部范围只搜一轮后返回穷举、仅裁剪负向越界范围，以及搜索续跑说明；续跑区保持直接显示。实际正负向范围、最近 16 个完成区域及每轮日志依赖正式进度 / 日志服务，Qt 继续显示未接入状态，不填充模拟数值。参考 `docs/TID_SEARCH.md`、`docs/TID_LOGGING.md` 与 Tk 的 `_refresh_tid_progress` / `clean_terminal_log`。
...
| 7 参数保存与搜索续跑 | 继续同参数的上次搜索进度（默认开启），覆盖穷举与非零半径乱数；进度含实际正负向范围、完成区域数与自动切换状态。Tk 自动保存，Qt 目前不保存 / 读取 |
...
七区功能归属均保留，独立脚本路径放在共通设置；主页面的范围、固定延迟和特殊号码默认折叠，随时可展开，与“高级模式”开关独立。“参数保存与搜索续跑”直接显示且不可折叠；新建存档警告与连续御三家条件也直接显示。
...
保留“孵蛋运行条件”（游戏、主机、Seed 模式、可编辑蛋种、完整准备/从已完成 254 步开始）与“孵蛋目标”（同 Seed、Held、Pickup、三档育成屋相性说明、双亲类型/六 IV、前置确认）。相性默认显示 Ten Lines 中文文案，标题旁按钮可按中文、英文、日文循环切换，内部和配置文件仍固定使用 20/50/70。亲本 A/B 均提供雄、雌、无性别和百变怪；合法组合与 Ten Lines 一致，只接受雄+雌或恰好一只百变怪。游戏和主机在 Tk 与野生/静态共用，Qt 表单也保持同步。
...
- 日志布局：设备标签诊断栏位于内容底部，日志卡片随窗口高度伸缩并占满其余空间；诊断展开后按内容需要分配高度，小窗口可滚动访问完整操作。
...
- 运行设置：HOME_BUFFER 自适应、命中后更新预校准、Seed 校准/启动直接放在顶部“高级模式”左侧，窄窗口分两行；脚本输出日志仍在共通设置。“高级模式｜设置”使用同行组合控件，右侧设置按钮打开正式/时间轴入口、奇偶和三层扩窗；包含层数 0–3 和各层 Seed/ADV 绝对半宽，仅普通、孵蛋或连续御三家适用，孵蛋菜单奇偶固定。TID 闪 PID、野生 SID 遍历起点和脚本测试从各自页面进入，不提供重复跳转按钮。
