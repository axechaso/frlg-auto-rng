...
| 操作与结果 | 2853–2879；`_on_mode_tab_change` | 右侧方案与运行准备；底栏直接显示生成、取消搜索、开始运行、停止 EasyCon；详情独立窗口。窄屏和整页实测表通过底栏打开同一摘要 | 后端操作全部禁用；结果不显示示例成功 |
...
| SID 查找脚本 | 独立 2.0 脚本包路径与选择，放在“共通设置 → SID 查找脚本”，不占 SID 主页面 | `sid_source_var` 独立于公共包路径；`write_sid_reverse_project`、`write_sid_reverse_plan`；运行器 `run_sid_reverse_capture.py` |
...
| 2 乱数中心 / 穷举范围 | OP/F1/F2 三列 × 中心帧、搜索半径、穷举起点、最大范围；当前 Tk 明确为 RNG advance（120.00144 advance/s）；起点默认 0。补充额外目标 TID、接近区间自动转乱数、距离 / 次数与切换后 OP/F1/F2 半径 |
...
| 6 TID 1.3.7 脚本包 | 独立包路径及选择，移至“共通设置 → TID 1.3.7 脚本包”，不占 TID 主页面 |
...
七区功能归属均保留，独立脚本路径放在共通设置；主页面的范围、固定延迟、特殊号码和续跑资料默认折叠，随时可展开，与“高级模式”开关独立。新建存档警告与连续御三家条件直接显示。
...
正式版/时间轴版/自选 ECS 入口、ECS 文件及选择、兼容/原始 CLI 后端、输出 EasyCon 详细日志、解析状态、常驻人工核对提示。Tk 的入口选择器位于公共区；Qt 复用高级设置中的同一份入口选择器，页内提供直接入口。
...
- 运行设置：HOME_BUFFER 自适应、命中后更新预校准、Seed 校准/启动直接放在顶部“高级模式”左侧，窄窗口分两行；脚本输出日志仍在共通设置。正式/时间轴入口、奇偶和三层扩窗移至开关旁的“高级设置…”窗口；包含层数 0–3 和各层 Seed/ADV 绝对半宽，仅普通、孵蛋或连续御三家适用，孵蛋菜单奇偶固定。窗口还可直接定位 TID 闪 PID、野生 SID 遍历起点和脚本测试。
...
悬停说明、高级模式完整范围及后续 TID 新增字段的复核见 [悬浮说明与高级模式核对](PYSIDE6_HELP_MAP.md)。本表开头保留初次基线来源，后续复核不再把旧预览视为当前 Tk 的完整实现。
