...
保留“孵蛋运行条件”（游戏、主机、Seed 模式、可编辑蛋种、完整准备/从已完成 254 步开始）与“孵蛋目标”（同 Seed、Held、Pickup、三档育成屋相性说明、双亲性别/六 IV、前置确认）。相性默认显示 Ten Lines 中文文案，标题旁按钮可切换英文，内部和配置文件仍固定使用 20/50/70。A 性别仅雌/无性别，B 仅雄/无性别。游戏和主机在 Tk 与野生/静态共用，Qt 表单也保持同步。
