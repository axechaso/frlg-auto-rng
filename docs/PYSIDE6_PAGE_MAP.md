...
保留“孵蛋运行条件”（游戏、主机、Seed 模式、可编辑蛋种、完整准备/从已完成 254 步开始）与“孵蛋目标”（同 Seed、Held、Pickup、三档育成屋相性说明、双亲类型/六 IV、前置确认）。相性默认显示 Ten Lines 中文文案，标题旁按钮可切换英文，内部和配置文件仍固定使用 20/50/70。亲本 A/B 均提供雄、雌、无性别和百变怪；合法组合与 Ten Lines 一致，只接受雄+雌或恰好一只百变怪。游戏和主机在 Tk 与野生/静态共用，Qt 表单也保持同步。
