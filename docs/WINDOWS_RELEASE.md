...
使用 `tools\build_windows_release.ps1` 生成发布包。构建机只需要项目自己的 `.venv` 和一次网络安装 PyInstaller；使用发布包的用户不需要安装 Python、pygame 或任何 Python 依赖。当前发布合同为 `0.2.1`。

```powershell
.\tools\build_windows_release.ps1 `
  -BuildTag updater-0-2-1 `
...
输出位于 `.build\windows-release-updater-0-2-1\FRLG-Auto-RNG-0.2.1-windows-x64`，同时生成同名 ZIP、`update-manifest.json` 和 `.sha256` 文件。发布包是绿色文件夹，不应把 `.venv`、源码或 Python 安装包一起复制给用户。配置、日志和运行时生成的 ECS 工程会写入 `%LOCALAPPDATA%\FRLG-Auto-RNG`。
...
`0.2.1` 包内包含 `FRLG-Auto-RNG-Updater.exe`。冻结绿色版启动后会在后台检查公开仓库的最新稳定 Release，每 24 小时最多自动检查一次；“检查程序更新”按钮可手动触发。发现新版本时只显示确认对话框，用户确认后才下载完整 ZIP，并校验清单、文件大小、SHA-256、ZIP 路径和内嵌版本。
...
现有 `1.1.8`、`0.2` 或更早绿色包没有本版修复，必须先手动解压安装 `0.2.1`；从 `0.2.1` 开始，后续正式 Release 才能使用应用内更新。源码运行模式不会联网自更新。
...
.\tools\publish_windows_release.ps1 `
  -BuildRoot .build\windows-release-updater-0-2-1 `
  -Tag v0.2.1 `
  -Title "FRLG Auto RNG 0.2.1 整包更新器版"
```
