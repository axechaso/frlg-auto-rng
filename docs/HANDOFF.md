...
孵化后的反查操作继续复用通用能力读取、IV 范围和吃糖后重新识图，并把队伍页上移抽为共享函数。普通定点新入队目标传 2；孵蛋正式轮的蛋在第 5 位、复核野生在第 6 位，传 3，只多一次 `UP`。Egg 的 Split/Normal/Alternate/Mixed 候选扫描仍是专用算法，不得误换为普通 Method 1/2/4。
...
下载包当前输入（含跨等级 IV 范围交集和终止时保留画面）SHA-256: bf3601815339f253ca0ee0b354fdfb2c26c07a8840f01e7cc375843a14e353b7
合并 1.6.4-a 修正后的当前基线 SHA-256: 30fea007607c06d69efdefe256c4b4a639d865854ca94da8afe309eaf0272451。上一基线 `1ea3bd0ba820e3cb3b1b8616f24e7e8d23b87767b23c49c77cc0a187c2037f73`、孵蛋野生反查窗口受控变体 `4843f4044e69dc4bc0eb2f3506490651589e531fe2d3b2bad905a6b977c3eec0` 和导入物化变体 `316c6aa9b6b05adeef0d7f306032b7ae553779d6a86848ae301aa981fd9a8188` 仍保留为历史兼容输入。
```
...
2026-08-24 的代码基线：项目 `.venv` 运行 165 项单元测试全部通过；本地资产正式版、本地资产时间轴版、当前运行副本、下载目录正式版和下载目录时间轴版共 5 份入口均通过真实 EasyCon 1.6.4-a `format`。下载包的 22 个 `Tools/check_*.py` 全部通过。当前下载包输入指纹为 `bf3601815339f253ca0ee0b354fdfb2c26c07a8840f01e7cc375843a14e353b7`，导入后物化的 33 文件固定 SHA-256 是 `30fea007607c06d69efdefe256c4b4a639d865854ca94da8afe309eaf0272451`；两处标签审计均为 1150 个标签、SHA-256 `00d2fbfa9a3638f3cea64553e94b777ed8c5c63f813125617b50aaeed7c9d10e`。本轮同步跨等级 IV 范围交集，并让最终无解直接保留画面停止；只有唯一候选的 Held/Pickup 校准误差仍会重启重试。日志包装器现在能区分正常终止与取消/异常提前退出。这只是代码、静态检查与语法验证，仍需 Switch 实机复测。确认：
...
测试基线是 165 项，下载包 22 个 ECS Tools 全部通过；五份主入口均通过真实 1.6.4-a `format`。1.1.8 语料会在导入本地快照时自动合并 1.6.4-a 修正，下载包输入指纹为 bf3601815339f253ca0ee0b354fdfb2c26c07a8840f01e7cc375843a14e353b7，当前固定语料为 33 个 ECS 文件、SHA-256 30fea007607c06d69efdefe256c4b4a639d865854ca94da8afe309eaf0272451。用户会直接修改下载包，未知指纹只警告后继续，不要擅自恢复严格拒绝；缺文件、标签或 format 失败仍应阻止。孵蛋与定点波克比已经共用固定周期孵化；孵化后普通新目标向上 2 次，孵蛋蛋向上 3 次；孵蛋多级 IV 范围取交集，最终无解直接保留游戏画面停止，只有唯一候选的 Held/Pickup 误差仍重启校准。下一步先做这两条流程的 Switch 实机验收。完成环境核对后，请根据我接下来的要求继续，不要自行扩大到修改 TID/EasyCon 原包。
```
