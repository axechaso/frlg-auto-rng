...
2026-08-25 更新的时间轴模板新增 `$孵蛋Held无蛋表Seed`、区间起止表等字段，生成器会按目标 Seed/Held/相性计算并写入。若报“模板字段实际为 0 次”，优先判断 `local_assets/easycon118` 是旧的忽略缓存并重新导入最新 1.1.8 包；不要把字段校验改为跳过。当前下载包输入指纹为 `bdd0ecbb9644555dd9adad4834ce61fa2ab343fe90df9d429de4be5fb8da6dbc`，导入后物化的 33 文件 SHA-256 为 `b0941989541991148e075926775f35bac301b524587048ba741a52f7f01da1b4`，旧物化指纹仍作为兼容输入保留。
...
下载包当前输入（Held 无蛋区间表）SHA-256: bdd0ecbb9644555dd9adad4834ce61fa2ab343fe90df9d429de4be5fb8da6dbc
合并 1.6.4-a 修正后的当前基线 SHA-256: b0941989541991148e075926775f35bac301b524587048ba741a52f7f01da1b4。上一基线 `74b4a3ecce59e3817699ee8dece2594d67f48bad08b33068358c45b74aaf6e9e` 及更早的已登记变体仍保留为历史兼容输入。
```
...
2026-08-25 的代码基线：项目 `.venv` 运行 199 项单元测试全部通过；内置 Seed 更新器已经用 Ten Lines 官方当前火红 2311 条、叶绿 2438 条数据完成真实联网验证，生成的两份 ECS 分别随正式版和孵蛋时间轴版通过真实 EasyCon 1.6.4-a `format`。更新后的本地资产正式版与时间轴版也通过真实 EasyCon 1.6.4-a `format`，下载包的 22 个 `Tools/check_*.py` 全部通过。当前下载包输入指纹为 `bdd0ecbb9644555dd9adad4834ce61fa2ab343fe90df9d429de4be5fb8da6dbc`，导入后物化的 33 文件固定 SHA-256 是 `b0941989541991148e075926775f35bac301b524587048ba741a52f7f01da1b4`；标签审计仍为 1150 个标签、SHA-256 `00d2fbfa9a3638f3cea64553e94b777ed8c5c63f813125617b50aaeed7c9d10e`。本轮新增内置 Seed 表更新、常驻运行日志页和开始运行自动跳转；此前的 Held 无蛋区间表写入、旧缓存明确诊断、亲本/全部两种孵蛋配置保存及运行中只读监视窗口继续保留。确认：
...
1. 在新设备重新跑安装、192 项测试、EasyCon `--check-only` 和设备枚举。
...
```text
最新补充（优先于下方历史快照数字）：当前测试基线为 192 项；最新 1.1.8 输入/物化指纹分别为 `bdd0ecbb...`/`b0941989...`，孵蛋页已有亲本配置与全部配置两套保存入口，运行脚本时监视窗口仍可打开；SID 查找已增加 Switch 1/2，并用 HOME_BUFFER 替换固定 A → 1200 ms → A，公共稳定低分自适应开关也作用于 SID 生成副本；命中后仍继续原 WAIT 8000、跳 OP 和进档操作。
