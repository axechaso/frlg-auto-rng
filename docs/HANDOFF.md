...
池塘 Seed 预校准的实机日志已证明冲浪流程存在转场竞态：同一 254 步基础档第 1/4 轮能进入战斗，第 2/3/5 轮固定动作执行完后却在非战斗画面得到 `LBOLT N`。运行时覆盖把冲浪确认的第三次 `A` 前等待从 800 ms 增至 2000 ms，并要求名称 OCR 前命中 `野生出现 > 90` 或 `抓捕就绪 > 95`；最多 4 次只等待重采样仍不成立就返回失败，让外层重启，检测期间不盲按键。后续排查必须优先看 `孵蛋池塘战斗检测|...` 日志，不能把这种状态错位归因于 OCR 或 1080p。
