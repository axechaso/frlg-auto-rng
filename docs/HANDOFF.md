...
2026-08-29 完整长跑日志证明两个联动缺陷：一是 Normal 首个非空结果会遮蔽 Split/Alternate/Mixed 中尚未排除的不同 IV；二是临时跳区后仍按原始 Held 目标排序，会在预测 `1127`、目标出蛋区间 `1128-1132` 时从 `1114/1126/1128` 错取 `1114`。当前时间轴缓存改为扫描并合并四种蛋方法的候选，只有全部候选完整保存且六项 IV 一致才停止喂糖；收敛后先选精确 Held/Pickup 目标，再优先本轮跳出目标区间，并按本轮跳出目标或“上次确认实际 Held + 本轮总修正变化”所得参考落点选择和复算方法。
...
本轮验证为下载包 30 个 `Tools/check_*.py`、工具 357 项单元测试全部通过；下载包正式/时间轴、工具缓存正式/时间轴和按 `EDDE / 1115 / 3405` 新生成的孵蛋工程共 5 份脚本通过真实 `1.6.4-a+9c86137` format。当前缓存 33 文件指纹为 `bde2ffddbb42b6c71b2494968c2ccfb8d04291ea3f3c6c755fa79ac825aba923`，整段包络版 `2ad7486f...`、前一版 `910667b1...` 和未加奇偶约束的中间版 `208cc097...` 保留为兼容输入；未重新打包 EXE、未运行 Switch 实机。
