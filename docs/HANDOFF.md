...
测试基线是 159 项。下载包 ECS Tools 为 20/23；三个已知陈旧失败固定是 check_egg_timeline_mode.py、check_seed_mode_calibration.py、check_seed_success_hold.py。1.1.8 原始审计语料会在导入本地快照时自动合并 1.6.4-a 修正，固定指纹见上文；不要绕过导入器混用未知脚本。完成环境核对后，请根据我接下来的要求继续，不要自行扩大到实机运行或修改 TID/EasyCon 原包。
```
