...
同日将完整同 Seed 孵蛋编排提升到正式 `1.1.8` 入口。两份入口的孵蛋主流程函数保持逐函数一致，并显式把 `$狩猎区TV时间轴测试` 传入 `lib/27`：正式版为 `0`，Seed、奇偶、TV、Held、Pickup 均使用普通 `WAIT`；时间轴版为 `1`，保留绝对截止和精确尾段。GUI 孵蛋生成器优先选择带 `FORMAL_EGG_WAIT_V1` 标记的正式模板；旧缓存没有该标记时才兼容回退到时间轴模板。
