...
合并 1.6.4-a 修正后的当前基线 SHA-256: da32012466a7349113ff166cf158c39dd721fc6e33c8d84355b9747cd7888f86。上一基线 `30fea007607c06d69efdefe256c4b4a639d865854ca94da8afe309eaf0272451` 及更早的已登记变体仍保留为历史兼容输入。
```
...
2026-08-24 的代码基线：项目 `.venv` 运行 165 项单元测试全部通过；本地资产正式版、本地资产时间轴版、当前运行副本、下载目录正式版和下载目录时间轴版共 5 份入口均通过真实 EasyCon 1.6.4-a `format`。下载包的 22 个 `Tools/check_*.py` 全部通过。当前下载包输入指纹为 `48cdfb839a81333e6115c72adc3ea40bf8642b091fbf55d7b49dac76cba4556f`，导入后物化的 33 文件固定 SHA-256 是 `da32012466a7349113ff166cf158c39dd721fc6e33c8d84355b9747cd7888f86`；两处标签审计均为 1150 个标签、SHA-256 `00d2fbfa9a3638f3cea64553e94b777ed8c5c63f813125617b50aaeed7c9d10e`。本轮同步 Held/Pickup 固定预校准、多候选轨迹和能力页闪光直判，并继续保留最终无解时的当前画面；已经确定同一套个体但 Held/Pickup 未命中时仍会重启校准。日志包装器能区分正常终止与取消/异常提前退出。这只是代码、静态检查与语法验证，仍需 Switch 实机复测。确认：
...
1. 在新设备重新跑安装、165 项测试、EasyCon `--check-only` 和设备枚举。
...
测试基线是 165 项，下载包 22 个 ECS Tools 全部通过；五份主入口均通过真实 1.6.4-a `format`。1.1.8 语料会在导入本地快照时自动合并 1.6.4-a 修正，下载包输入指纹为 48cdfb839a81333e6115c72adc3ea40bf8642b091fbf55d7b49dac76cba4556f，当前固定语料为 33 个 ECS 文件、SHA-256 da32012466a7349113ff166cf158c39dd721fc6e33c8d84355b9747cd7888f86。用户会直接修改下载包，未知指纹只警告后继续，不要擅自恢复严格拒绝；缺文件、标签或 format 失败仍应阻止。当前工具工作区是 D:\Codex\火叶乱数\frlg-auto-rng。孵蛋与定点波克比已经共用固定周期孵化；孵蛋 Held/Pickup 固定预校准均为 230，Held/Pickup 反查窗分别为 ±100/±2000，多候选按相同 IV 和跨轮轨迹处理；最终无解保留游戏画面停止，已确定个体但目标帧未命中时重启校准。下一步先做这两条流程的 Switch 实机验收。完成环境核对后，请根据我接下来的要求继续，不要自行扩大到修改 TID/EasyCon 原包。
```
