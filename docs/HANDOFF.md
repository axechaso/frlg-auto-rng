...
上一版合并语料（可自动升级）SHA-256: aea14e79615bfda89e1f7428014adc2dcc848005bd7ebad0bd170eac67703aef
合并 1.6.4-a 修正后的固定 SHA-256: fe0ae41be3fe035cbefec9afd525b968a070c8781a73595e1ae16b4cd1e2e839
```
...
1. 在新设备重新跑安装、158 项测试、EasyCon `--check-only` 和设备枚举。
...
测试基线是 158 项。下载包 ECS Tools 为 20/23；三个已知陈旧失败固定是 check_egg_timeline_mode.py、check_seed_mode_calibration.py、check_seed_success_hold.py。1.1.8 原始审计语料会在导入本地快照时自动合并 1.6.4-a 修正，固定指纹见上文；不要绕过导入器混用未知脚本。完成环境核对后，请根据我接下来的要求继续，不要自行扩大到实机运行或修改 TID/EasyCon 原包。
```
