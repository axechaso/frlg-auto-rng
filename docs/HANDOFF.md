...
2026-08-22 后续把孵蛋页与定点波克比的孵化执行合并为共享函数。两者都按 `(孵化周期 + 1) × 256 - 1`、每轮 38 步和 1 轮安全余量计算固定循环，不再读取 `蛋孵化` 标签；波克比的 10 周期得到 76 轮。完成后统一复用原定点波克比的 `WAIT 500 → B → WAIT 12000 → B → WAIT 500` 收尾。波克比自己的领取对话、F2 时间点和 TV/上车分支没有改变。
...
合并 1.6.4-a 修正后的固定 SHA-256: 43c3944bad75a1cb424203237b6aad51b351aa5c9bb81bfc6aa2c93ce96932cf
```

安装器只接受上述原包输入、明确列出的上一版合并语料或当前固定语料；复制到 `local_assets/easycon118/` 后都会自动升级并按新指纹复核。两份入口和共享 `lib` 因而可以直接运行，不再依赖 GUI 临时覆盖。不要把曾经的旧快照指纹 `db50e6...` 或有 `$调试日志输出` 编译问题的其他快照直接替换进来。
...
2026-08-22 的代码基线：项目 `.venv` 运行 159 项单元测试全部通过；本地资产正式版、本地资产时间轴版、当前运行副本、下载目录正式版和下载目录时间轴版共 5 份入口均通过真实 EasyCon 1.6.4-a `format`。下载包共有 23 个 `Tools/check_*.py`，当前 20 个通过；以下 3 个是外部下载包对旧函数/文案的已知陈旧断言：`check_egg_timeline_mode.py`、`check_seed_mode_calibration.py`、`check_seed_success_hold.py`。换设备复测必须同时报告“20/23”和这三个文件名，不能简写成全部通过。本轮把用户制作的 `冲浪.IL`（规范文件 SHA-256 `948b8a57ccabfb86d5c358208d7d2a944a8133a56a58ba24d6ce5fafb0d04fe9`）纳入仓库扩展和 1150 标签审计语料；孵蛋池塘流程只循环读取这个标签，首次 `> 95` 后立即停止识图、按一次 `X` 并沿用原版 `WAIT 500 → DOWN`，不再读取 `三代菜单栏` 或御三家用的 `火红BAG`。瞬时动作失败在完成重启后返回 `2` 进入下一轮，不再错误返回 `0` 终止总控。孵蛋页和定点波克比现已共用固定周期骑车及两次 `B` 收尾，波克比不再依赖孵化标签；孵化后队伍选择也复用通用上移函数，孵蛋蛋只多一次 `UP`。以上修正已经固化到导入后的两份 1.1.8 入口和共享库，生成器仅做幂等配置；当前下载目录脚本也已同步。脚本语料为 33 个 ECS 文件，固定 SHA-256 是 `43c3944bad75a1cb424203237b6aad51b351aa5c9bb81bfc6aa2c93ce96932cf`。这只是代码、静态检查与语法验证，仍需 Switch 实机复测。SID 第一只闪光雄性大比鸟曾完成两轮连续实机采集，均得到 Lv46、Jolly、`133/120/83/72/73/144`；此前还完成四个正式页签顺序、默认隐藏高级页、SID 输入/运行链、御三家目标搜索和英文三阶段连续流程生成/预检的静态验证。确认：
...
1. 在新设备重新跑安装、159 项测试、EasyCon `--check-only` 和设备枚举。
...
测试基线是 159 项。下载包 ECS Tools 为 20/23；三个已知陈旧失败固定是 check_egg_timeline_mode.py、check_seed_mode_calibration.py、check_seed_success_hold.py。1.1.8 原始审计语料会在导入本地快照时自动合并 1.6.4-a 修正，当前固定语料为 33 个 ECS 文件、SHA-256 43c3944bad75a1cb424203237b6aad51b351aa5c9bb81bfc6aa2c93ce96932cf；不要绕过导入器混用未知脚本。最近已验证落点为 602440c，对应 Actions 32560865715 成功。孵蛋与定点波克比已经共用固定周期孵化；孵化后普通新目标向上 2 次，孵蛋蛋向上 3 次，Egg 仍使用专用候选算法。下一步先做这两条流程的 Switch 实机验收。完成环境核对后，请根据我接下来的要求继续，不要自行扩大到修改 TID/EasyCon 原包。
```
