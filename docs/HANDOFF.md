...
12. 公共设置中“高级模式”旁有 HOME_BUFFER 稳定低分自适应开关。它只影响 1.1.8 普通/孵蛋入口且默认关闭；TID 1.3.7 原控制器不得改写。
...
GUI 默认有四个选项卡，固定顺序为“SID 查找”“TID 乱数”“野生 / 静态”“孵蛋（测试）”。公共 EasyCon 设置中的串口/采集卡使用启动自动检测后填充的下拉框，采集卡同时显示序号和设备名。“高级模式”旁的 HOME_BUFFER 稳定低分自适应默认关闭；开启后生成器才为 1.1.8 普通/孵蛋脚本写入开关。勾选“高级模式”后，才会显示第五个“脚本测试（高级）”页：
...
正式版与时间轴版共享 `home_buffer_adaptive_classifier.ecs`。默认有效阈值为 95；只有用户开启开关、当前 HOME_BUFFER 状态是三类标签中的唯一最高、分数至少 90，并且连续 3 次状态与整数分数完全相同，才把本次运行的有效阈值降到该分数。日志输出 `HOME_BUFFER_ADAPTIVE|OLD=95|NEW=...`。不得把它扩展到普通 OCR/能力/冲浪标签，也不得修改 TID 1.3.7 的 HOME_BUFFER。
...
下载包当前输入（含固定预校准和多候选轨迹）SHA-256: 48cdfb839a81333e6115c72adc3ea40bf8642b091fbf55d7b49dac76cba4556f
合并 1.6.4-a 修正后的当前基线 SHA-256: 1700ba02cc60fdfd9857f14a2a8384c5736c06908a92e468d1dfd721a9be4865。加入 HOME_BUFFER 共享分类器前的基线 `da32012466a7349113ff166cf158c39dd721fc6e33c8d84355b9747cd7888f86` 及更早的已登记变体仍保留为历史兼容输入。
```
...
2026-08-24 的代码基线：项目 `.venv` 运行 170 项单元测试全部通过；本地资产正式版、本地资产时间轴版、当前运行副本、下载目录正式版和下载目录时间轴版共 5 份入口，以及显式打开 HOME_BUFFER 自适应后的两份临时入口，均通过真实 EasyCon 1.6.4-a `format`。下载包的 22 个 `Tools/check_*.py` 全部通过。当前下载包输入指纹为 `48cdfb839a81333e6115c72adc3ea40bf8642b091fbf55d7b49dac76cba4556f`，导入后物化的 33 文件固定 SHA-256 是 `1700ba02cc60fdfd9857f14a2a8384c5736c06908a92e468d1dfd721a9be4865`；两处标签审计均为 1150 个标签、SHA-256 `00d2fbfa9a3638f3cea64553e94b777ed8c5c63f813125617b50aaeed7c9d10e`。本轮新增启动自动设备检测、串口/带名称采集卡下拉框，以及默认关闭的 HOME_BUFFER 稳定低分自适应；真实枚举得到 `COM5` 和 4 个带名称视频设备。这只是代码、静态检查与语法验证，仍需 Switch 实机复测。确认：

- 页签顺序固定为 SID 查找、TID 乱数、野生/静态、孵蛋；
- 设备枚举会自动回填可用串口和带名称采集卡，并保留仍在线的当前索引；
- HOME_BUFFER 自适应只写入 1.1.8 普通/孵蛋入口，默认关闭；
- 普通类型下拉框不再包含孵蛋；
...
1. 在新设备重新跑安装、170 项测试、EasyCon `--check-only` 和设备枚举。
...
测试基线是 170 项，下载包 22 个 ECS Tools 全部通过；五份主入口和两份开启 HOME_BUFFER 自适应的临时入口均通过真实 1.6.4-a `format`。1.1.8 语料会在导入本地快照时自动合并 1.6.4-a 修正，下载包输入指纹为 48cdfb839a81333e6115c72adc3ea40bf8642b091fbf55d7b49dac76cba4556f，当前固定语料为 33 个 ECS 文件、SHA-256 1700ba02cc60fdfd9857f14a2a8384c5736c06908a92e468d1dfd721a9be4865。用户会直接修改下载包，未知指纹只警告后继续，不要擅自恢复严格拒绝；缺文件、标签或 format 失败仍应阻止。当前工具工作区是 D:\Codex\火叶乱数\frlg-auto-rng。GUI 启动后自动枚举串口和采集卡；采集卡下拉项显示序号与设备名。高级模式旁的 HOME_BUFFER 稳定低分自适应只作用于 1.1.8 普通/孵蛋且默认关闭，连续 3 次完全相同的唯一最高 90–94 分才可降低本次阈值。SID 采集日志会逐行落盘并在主界面实时回显，取消不得再覆盖成空日志。孵蛋与定点波克比已经共用固定周期孵化；孵蛋 Held/Pickup 固定预校准均为 230，Held/Pickup 反查窗分别为 ±100/±2000，多候选按相同 IV 和跨轮轨迹处理；最终无解保留游戏画面停止，已确定个体但目标帧未命中时重启校准。下一步先做这两条流程的 Switch 实机验收。完成环境核对后，请根据我接下来的要求继续，不要自行扩大到修改 TID/EasyCon 原包。
```
