...
2026-08-24 的代码基线：项目 `.venv` 运行 167 项单元测试全部通过；本地资产正式版、本地资产时间轴版、当前运行副本、下载目录正式版和下载目录时间轴版共 5 份入口均通过真实 EasyCon 1.6.4-a `format`。下载包的 22 个 `Tools/check_*.py` 全部通过。当前下载包输入指纹为 `48cdfb839a81333e6115c72adc3ea40bf8642b091fbf55d7b49dac76cba4556f`，导入后物化的 33 文件固定 SHA-256 是 `da32012466a7349113ff166cf158c39dd721fc6e33c8d84355b9747cd7888f86`；两处标签审计均为 1150 个标签、SHA-256 `00d2fbfa9a3638f3cea64553e94b777ed8c5c63f813125617b50aaeed7c9d10e`。本轮还修复 SID 单槽未结束前日志不落盘、取消后被空文件覆盖以及失败后主界面不回显的问题；原始日志仍保留，界面显示会去除 ANSI 控制码。这只是代码、静态检查与语法验证，仍需 Switch 实机复测。确认：
...
1. 在新设备重新跑安装、167 项测试、EasyCon `--check-only` 和设备枚举。
